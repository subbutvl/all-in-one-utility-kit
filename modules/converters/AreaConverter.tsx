    ...
    <div className="h-full flex flex-col gap-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 min-h-0">
        <div className="lg:col-span-4 bg-[#111] border border-neutral-800 p-8 space-y-6 h-fit">
          <label className="text-[10px] uppercase text-neutral-500 font-bold mb-1 block tracking-widest">Input Area</label>
        ...
        <div className="lg:col-span-8 overflow-y-auto pb-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {results.map(r => (
              <div key={r.unit} className="bg-[#111] border border-neutral-800 p-4 flex justify-between items-center group hover:border-neutral-600 transition-all">
                <div>
                  <div className="text-[9px] uppercase text-neutral-600 font-bold mb-1 tracking-widest">{r.unit}</div>
                  <div className="text-sm font-mono text-neutral-200">{r.value.toLocaleString(undefined, { maximumFractionDigits: 8 })}</div>
                </div>
              </div>
            ))}
          </div>
