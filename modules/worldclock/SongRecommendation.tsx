...
const STATIC_SONGS: Song[] = [
  { title: "Midnight City", artist: "M83", genre: "Electronic", description: "An iconic synth-pop anthem with a signature saxophone riff." },
  { title: "Weightless", artist: "Marconi Union", genre: "Lofi", description: "Scientific evidence suggests this is the world's most relaxing song." },
  { title: "Bohemian Rhapsody", artist: "Queen", genre: "Rock", description: "A six-minute suite, consisting of several sections without a chorus." },
  { title: "Blinding Lights", artist: "The Weeknd", genre: "Pop", description: "A masterclass in 80s synth revivalism." }
];

export const SongRecommendation: React.FC = () => {
  const [primary, setPrimary] = useState<Song | null>(null);
  const [secondary, setSecondary] = useState<Song[]>([]);
  const [loading, setLoading] = useState(false);
  const [mode, setMode] = useState<'static' | 'ai'>('static');
  const [category, setCategory] = useState('Electronic');

  const categories = ['Pop', 'Rock', 'Jazz', 'Classical', 'Lofi', 'Electronic'];

  const fetchStatic = () => {
    setLoading(true);
    setTimeout(() => {
      const pool = STATIC_SONGS.filter(s => s.genre === category || category === 'All').length > 0 ? STATIC_SONGS.filter(s => s.genre === category) : STATIC_SONGS;
  ...
  const fetchAI = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Recommend 4 songs in the "${category}" genre. One primary and three additional. Return as JSON array of objects with 'title', 'artist', 'genre', 'description'.`,
        config: {
        ...
        {primary ? (
          <section className="bg-[#0d0d0d] border border-neutral-800 p-10 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-100 transition-opacity">
               <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-white"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
            </div>
            <div className="max-w-3xl space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] text-neutral-600 font-mono">Artist: {primary.artist}</span>
              </div>
          ...
          {secondary.map((s, i) => (
            <div key={i} className="bg-[#111] border border-neutral-800 p-6 space-y-3 hover:border-neutral-600 transition-colors">
              <div className="flex justify-between items-start">
                ...
                <span className="text-[9px] text-neutral-700 font-mono">/ {s.artist}</span>
              </div>
