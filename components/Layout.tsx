    ...
    <div className="h-screen w-screen flex flex-col bg-[#0a0a0a] text-neutral-200 select-none">
      ...
      <nav className="h-12 border-b border-neutral-800 flex items-center px-4 gap-2 z-50 bg-[#0a0a0a]">
        ...
        >
          ...
          <span className="font-bold tracking-tight text-[16px]">SWISS KNIFE</span>
        </div>
        ...
        <div className="flex items-center h-full">
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                <button className={`px-3 h-full text-[15px] font-medium transition-all ${
                  openCategory === cat 
                  ...
                  >
                    ...
                    {getToolsByCategory(cat).length > 0 ? (
                      <div className="flex flex-col">
                        <div className={`px-3 py-2 text-[12px] uppercase tracking-[0.2em] font-bold border-b border-neutral-800 mb-1 ${isSpecial ? 'text-indigo-400' : 'text-neutral-600'}`}>
                        ...
                        {getToolsByCategory(cat).map(tool => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2.5 text-[15px] flex items-center gap-3 hover:bg-neutral-800 transition-colors ${activeToolId === tool.id ? 'bg-neutral-800 text-white' : 'text-neutral-400 hover:text-white'}`}
                          >
                      ...
                      <div className="px-4 py-3 text-[12px] uppercase tracking-widest text-neutral-600 italic">No tools developed yet</div>
                    )}
                  </div>
                )}
              </div>
        ...
        <div className="flex items-center gap-4 text-neutral-500 mr-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] font-mono opacity-50 uppercase tracking-tighter">v1.0.0-BETA</span>
            </div>
        </div>
      </nav>
      ...
      <footer className="h-6 border-t border-neutral-800 bg-[#070707] flex items-center px-4 justify-between text-[11px] text-neutral-600">
        ...
        <div className="flex items-center gap-3">
          <span className="uppercase tracking-widest text-[11px] font-bold">System Tray Active</span>
